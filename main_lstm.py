...
def main():
    ...
    # Validation split from tail of training data
    val_frac = 0.15
    ...
    results = pd.DataFrame(
        {
            "Date": df_test["Date"].iloc[seq_len - 1:].reset_index(drop=True),
            "y_true": y_test_seq_true,
            "pred_hist": pred_hist[seq_len - 1:],
            "pred_ewma": pred_ewma[seq_len - 1:],
            "pred_lstm": pred_lstm,
        }
    )
        ...
        [
            {
                ...
                "RMSE": rmse(y_test_seq_true, pred_hist[seq_len - 1:]),
                "MAE": mae(y_test_seq_true, pred_hist[seq_len - 1:]),
            },
            {
                "Model": "EWMA",
                "RMSE": rmse(y_test_seq_true, pred_ewma[seq_len - 1:]),
                "MAE": mae(y_test_seq_true, pred_ewma[seq_len - 1:]),
            },
            {
                "Model": "LSTM",
